...
uint64_t zobristBoard[48][8];
uint64_t zobristComputerTurn;
uint64_t hashT[65536][3]; // key, depth, score
unsigned int minmaxcalls = 0;
...
uint64_t getZobristHash(bool comp){
  uint64_t returnKey = 0ULL;
  for(int square = 0; square < 48; square++){
    if(((board[3] >> square) & 1) == 1){ // computer pawn found
      returnKey ^= zobristBoard[square][3];
    }
    else if(((board[7] >> square) & 1) == 1){ // human pawn found
      returnKey ^= zobristBoard[square][7];
    }
    else if(((board[0] >> square) & 1) == 1){ // computer horse found
      returnKey ^= zobristBoard[square][0];
    }
    else if(((board[4] >> square) & 1) == 1){ // human horse found
      returnKey ^= zobristBoard[square][4];
    }
    else if(((board[2] >> square) & 1) == 1){ // computer bishop found
      returnKey ^= zobristBoard[square][2];
    }
    else if(((board[6] >> square) & 1) == 1){ // human bishop found
      returnKey ^= zobristBoard[square][6];
    }
    else if(((board[1] >> square) & 1) == 1){ // computer king found
      returnKey ^= zobristBoard[square][1];
    }
    else if(((board[5] >> square) & 1) == 1){ // human king found
      returnKey ^= zobristBoard[square][5];
    }
  }
  if(comp){
    returnKey ^= zobristComputerTurn;
  }
  return returnKey;
}

unsigned int isInZobTable(uint64_t key, bool comp){
  unsigned short hash = key % 65536;
  if(hashT[hash][0] == key)
    return hash;
  else return 100000;
}
        ...
        if (searchindex < 48 && searchindex > -1 &&
          ...
          // check senior pieces
            ...
            if(allpieces & moveMask){
              ...
              if((comp && moveMask & hpieces) || (!comp && moveMask & cpieces)){
                capturing = true;
                bmoves[moveIndex] = getMoveFromMask(mask, moveMask);
                moveIndex++;
              }
...
bool gameOver(uint64_t board[], short pActive[], int *hmoves, int *cmoves){
  // no computer kings remain
  if(pActive[1] == 0){
    return true;
  }
...
int max(uint64_t board[], uint16_t allmoves[][56], short pActive[], short maxdepth, short depth, unsigned int *leafnodes, unsigned int *nonroot, std::chrono::high_resolution_clock::time_point t1, int alpha, int beta, bool comp)
{
  minmaxcalls+=1;
  int cmoves, hmoves = 0;
  if(gameOver(board, pActive, &hmoves, &cmoves)){
    *leafnodes+=1;  return evalend(board, pActive, depth, t1, true, hmoves, cmoves);
  }
  else if(maxdepth == depth) {
    *leafnodes+=1; return eval(board, pActive, depth, t1, true, hmoves, cmoves);
  }
  else{
    int myalpha = alpha;
    int mybeta = beta;
    ...
    *nonroot += len;
	...
	for(int i = 0; i < noncapture; i++){
      ...
      uint64_t key = getZobristHash(comp);
      unsigned int hash = isInZobTable(key, comp);
      // hash was found in table and matches
      if(hash < 65536){
        if((short)hashT[hash][1] >= depth){
          score = hashT[hash][2];
        }
        else{
          score = min(board, allmoves, pActive, maxdepth, depth+1, leafnodes, nonroot, t1, myalpha, mybeta, !comp);
          hashT[hash][1] = depth;
          hashT[hash][2] = score;
        }
      }
      //hash was not found or there was a collision
      else{
        hash = key % 65536;
...
int min(uint64_t board[], uint16_t allmoves[][56], short pActive[], short maxdepth, short depth, unsigned int *leafnodes, unsigned int *nonroot, std::chrono::high_resolution_clock::time_point t1, int alpha, int beta, bool comp)
{
  ...
  int cmoves, hmoves = 0;
  if(gameOver(board, pActive, &hmoves, &cmoves)){
    *leafnodes+=1; return evalend(board, pActive, depth, t1, comp, hmoves, cmoves);
  }
  else if(maxdepth == depth) {
    *leafnodes+=1; return eval(board, pActive, depth, t1, comp, hmoves, cmoves);
  }
  else{
    int myalpha = alpha;
    int mybeta = beta;
    ...
    short len = generateMoves(board, allmoves[depth], comp, pActive);
    *nonroot += len;
    short noncapture = 0;//orderMoves(board, pActive, allmoves,len, depth, comp);
	...
	for(int i = 0; i < noncapture; i++){
      ...
      cout << hash  << endl;//<< "\t" << hashT[hash].key << "\t" << hashT[hash].depth << "\t" << hashT[hash].score << endl;
      // hash was found in table and matches
      if(hash < 65536){
        if((short)hashT[hash][1] >= depth){
          score = hashT[hash][2];
          ...
          score = max(board, allmoves, pActive, maxdepth, depth+1, leafnodes, nonroot, t1, myalpha, mybeta, !comp);
          hashT[hash][1] = depth;
          hashT[hash][2] = score;
        }
      }
...
uint16_t minimax(uint64_t board[], short pActive[], std::chrono::high_resolution_clock::time_point t1, bool comp){
  predicted = false;
  // best move played in minimax thus far
  ...
  for(int k = 1; k < 30 ; k++){
    ...
    // reset the hash table after each iteration
    for(int i = 0; i < 65536; i++){
      hashT[i][1] = -1;
      hashT[i][0] = 0;
      hashT[i][2] = 0;
  ...
  cout << "Avg Branching Factor: " << (double) (nonroot) / minmaxcalls << endl;
...
int main()
{
  ...
  while(gameOver(board, pActive, &hmoves, &cmoves) == false){
    // Computer goes
    ...
    if(gameOver(board, pActive, &hmoves, &cmoves) == false){
      // user goes
      userTurn(board, pActive, movesPlayed, moveNumber);
      moveNumber++;
